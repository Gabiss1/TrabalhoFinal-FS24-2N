...
public class ListarPokesPanel extends JInternalFrame {
    ...
    private void buscarPokemonsPorNome() {
        ...
        List<Pokemon> pokemons = controller.getPokemonByName(nomeBusca);
    ...
    private void editarPokemonSelecionado() {
        ...
        if (selectedRow >= 0) {
            ...
            CadastrarPoke cadastrarPoke = new CadastrarPoke(controller, idPokemon);
            this.getDesktopPane().add(cadastrarPoke);
            cadastrarPoke.setVisible(true);
            cadastrarPoke.toFront();

            cadastrarPoke.addInternalFrameListener(new javax.swing.event.InternalFrameAdapter() {
                @Override
